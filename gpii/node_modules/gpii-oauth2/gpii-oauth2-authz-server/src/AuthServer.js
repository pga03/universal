...
gpii.oauth2.authServer.contributeRouteHandlers = function (that, oauth2orizeServer, passport) {
    ...
    that.expressApp.post("/add-preferences",
        function (req, res) {
            fluid.log("POST /add-preferences: entering");
            fluid.log("POST /add-preferences: request body: " + JSON.stringify(req.body, null, 2));
            fluid.log("POST /add-preferences: request query: " + JSON.stringify(req.query, null, 2));


            var accessToken = gpii.oauth2.parseBearerAuthorizationHeader(req);
            if (!accessToken) {
                fluid.log("POST /add-preferences: the accessToken does not exist");
                res.send(401);
            } else {
                var client = that.authorizationService.getClientByClientCredentialsAccessToken(accessToken);
                if (!client) {
                    fluid.log("POST /add-preferences: the client does not exist");
                    res.send(404);
                }
                ...
                if (!tokenPrivs || !tokenPrivs.allowAddPrefs) {
                    fluid.log("POST /add-preferences: the token privileges do not exist or does not contain allowAddPrefs");
                    res.send(401);
                } else {
                    fluid.log("POST /add-preferences: calling on that.authorizationService.savePrefs");
                    var savePrefsPromise = that.authorizationService.savePrefs(req.body, req.query.view);
                    savePrefsPromise.then(function (data) {
                        fluid.log("POST /add-preferences: savePrefsPromise has resolved with data: " + JSON.stringify(data, null, 2));
                        res.json(data);
                    }, function (err) {
                        fluid.log("POST /add-preferences: savePrefsPromise has rejected with error: " + JSON.stringify(err, null, 2));
