/*!
GPII Preferences Server GET Handler

Copyright 2014 Raising the floor - International

Licensed under the New BSD license. You may not use this file except in
compliance with this License.

The research leading to these results has received funding from the European Union's
Seventh Framework Programme (FP7/2007-2013) under grant agreement no. 289016.

You may obtain a copy of the License at
https://github.com/GPII/universal/blob/master/LICENSE.txt
*/

(function () {
    "use strict";

    var fluid = require("infusion"),
        gpii = fluid.registerNamespace("gpii");

    fluid.registerNamespace("gpii.preferencesServer.get");

    fluid.defaults("kettle.requests.request.handler.preferencesGet", {
        gradeNames: ["autoInit", "fluid.littleComponent"],
        invokers: {
            handle: {
                funcName: "gpii.preferencesServer.get.getRawPreferences",
                args: [ "{that}", "{preferencesServer}", "{request}"],
                dynamic: true
            },
            buildReturnPayload: {
                funcName: "gpii.preferencesServer.get.buildReturnPayload",
                args: [ "{arguments}.0", "{preferencesServer}.ontologyHandler", "{request}"]
            }
        }
    });

    gpii.preferencesServer.get.getRawPreferences = function (that, preferencesServer, request) {
        fluid.log("gpii.preferencesServer.get.getRawPreferences: entering");
        fluid.log("gpii.preferencesServer.get.getRawPreferences preferencesServer: " + JSON.stringify(preferencesServer, null, 2));
        fluid.log("gpii.preferencesServer.get.getRawPreferences request.req.body: " + JSON.stringify(request.req.body, null, 2));

        fluid.log("gpii.preferencesServer.get.getRawPreferences: creating promise for preferencesServer.getRawPreferences for userToken: " + request.req.params.userToken);
        var promise = preferencesServer.getRawPreferences(request.req.params.userToken);
        promise.then(function (rawPrefs) {

            if (!fluid.isDestroyed(request)) { // TODO: Remove this ugly check when there is a solution for FLUID-5790
                fluid.log("gpii.preferencesServer.get.getRawPreferences promise for preferencesServer.getRawPreferences is resolved with rawPrefs: " + JSON.stringify(rawPrefs, null, 2));;
                that.buildReturnPayload(rawPrefs);
            }
        }, request.events.onError.fire);
    };

    gpii.preferencesServer.get.buildReturnPayload = function (rawPrefs, ontologyHandler, request) {
        fluid.log("gpii.preferencesServer.get.buildReturnPayload: entering");

        fluid.log("Raw preferences fetched by PreferencesServer: " + JSON.stringify(rawPrefs, null, 2));
        // grab the desired view (ontology) value from parameter (default to "flat") and GET
        var toView = gpii.preferencesServer.getRequestView(request);
        var prefs = ontologyHandler.rawPrefsToOntology(rawPrefs, toView);
        request.events.onSuccess.fire(prefs);
    };
})();